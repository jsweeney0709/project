# -*- coding: utf-8 -*-
"""
Created on Sun Dec  5 18:09:48 2021
...
img=Image.open("C:/Users/jswee/OneDrive/Desktop/Project/InterstellarWormhole_Fig10.jpg")# e.g. C:/Users..../stars.jpg
...
lList=np.linspace(newbiggest,1.56,400)
leval=np.linspace(newbiggest,1.56,800)
...
#x*pi/height,y*pi/height


#for each pixel in the image look at a ray being sent to that source with a black hole at the origin

for j in range(height):
    for i in range(-int(((height/2)**2-(j-halfHeight)**2)**(1/2)),int(((height/2)**2-(j-halfHeight)**2)**(1/2))):
        x=i
        pixx=int(halfWidth+i)
        y=j-halfHeight
        pixy=j
        if x!=0:
            p=(x/abs(x))*(1.55/halfHeight)*(x**2+y**2)**(1/2)
        if x==0:
            p=(1.55/halfHeight)*(x**2+y**2)**(1/2)
...
img.save("C:/Users/jswee/.spyder-py3/Dissertation/hdri.jpg")        
