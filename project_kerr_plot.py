...
@author: jswee
"""
from numpy import sin, cos, tan, pi, sqrt, mgrid, floor
...
def Sigma(r,theta):
    modulo = floor(theta/(2*pi))
    return(a*cos(theta-2*pi*modulo))**2+r**2
def drSigma(r):
    return 2*r
def dthetaSigma(theta):
    modulo = floor(theta/(2*pi))
    return -2*cos(theta-2*pi*modulo)*sin(theta-2*pi*modulo)*a**2
def Delta(r):
    return a**2+r**2-2*r
def drDelta(r):
    return 2*r-2
...
def mydiff(lambd,x):
    ...
    f4=-(1/(2*Delta(x[0])**2*Sigma(x[0],x[1])**2))*(Sigma(x[0],x[1])*(-E*Delta(x[0])*(a*2*(-2*L+a*E*sin(x[1])**2)+2*x[0]*E*Sigma(x[0],x[1]))+(a*(a*L**2-2*L*x[0]*2*E+a*x[0]*2*E**2*sin(x[1])**2)+x[3]**2*Delta(x[0])**2+(a**2+x[0]**2)*E**2*Sigma(x[0],x[1]))*drDelta(x[0]))+Delta(x[0])*(a*(L*(a*L-2*x[0]*2*E)+a*x[0]*2*E**2*sin(x[1])**2)-Delta(x[0])*(x[4]**2+(L/sin(x[1]))**2+x[3]**2*Delta(x[0])))*drSigma(x[0]))
    f5=-1/(2*Delta(x[0])*Sigma(x[0],x[1])**2)*(-2*sin(x[1])*(x[0]*2*cos(x[1])*(E*a)**2+(Delta(x[0])*L**2)/(tan(x[1])*sin(x[1])**3))*Sigma(x[0],x[1])+(a*(L*(a*L-2*x[0]*2*E)+a*x[0]*2*(sin(x[1])*E)**2)-Delta(x[0])*(x[4]**2+(L/sin(x[1]))**2+Delta(x[0])*x[3]**2))*dthetaSigma(x[1]))
